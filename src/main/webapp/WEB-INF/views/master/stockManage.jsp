...
$(function() {
    ...
    //레이어팝업 : 재고조사 Layer 팝업
    function fcStock_CheckForm(){

    	$('#stockCheckForm').dialog({
            resizable : false, //사이즈 변경 불가능
            draggable : true, //드래그 불가능
            closeOnEscape : true, //ESC 버튼 눌렀을때 종료
            ...
            open:function(){
                //팝업 가져올 url
                $(this).load('<%= request.getContextPath() %>/master/stockcheckform');
                //$("#userRegist").dialog().parents(".ui-dialog").find(".ui-dialog-titlebar").hide();
                $(".ui-widget-overlay").click(function(){ //레이어팝업외 화면 클릭시 팝업 닫기
                    $("#stockCheckForm").dialog('close');

                    });
            }
            ,close:function(){
                $('#stockCheckForm').empty();
            }
        });
    };
</SCRIPT>
<div class="container-fluid">
  ...
  <button type="button" class="btn btn-success" onClick="fcStock_CheckForm()">재고조사</button>
  </c:if> 
  <!-- 재고현황 일괄등록-->
  <div id="stockExcelForm"  title="재고현황 일괄등록"></div>
  <!-- //재고현황 일괄등록 -->
  <!-- 재고현황 조사-->
  <div id="stockCheckForm"  title="재고조사"></div>
  <!-- //재고현황 일괄등록 -->
   <!-- 재고상세현황 페이지-->
